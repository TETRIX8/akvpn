...
export const PurchasePopup: React.FC<PurchasePopupProps> = ({ isOpen, onClose }) => {
  ...
  const handleWhatsAppClick = () => {
    let message = "Здравствуйте! Хочу приобрести VPN.";
    if (selectedPlan) {
      message += ` На ${selectedPlan.duration} за ${selectedPlan.price} ₽.`;
    }
            ...
            {pricingPlans.map((plan, index) => (
              <div 
                ...
                className={`flex justify-between items-center p-3 rounded-lg cursor-pointer transition-all ${
                  selectedPlan === plan 
                    ? "bg-gradient-to-r from-vpn-blue/50 to-purple-600/50 border border-vpn-blue" 
                    : plan.highlight 
              ...
              >
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    {(selectedPlan === plan || plan.highlight) && <Check className="h-4 w-4 text-vpn-blue" />}
                    <span className="font-medium text-white">{plan.duration}</span>
                  </div>
                  {plan.description && (
                    <span className="text-xs text-white/60 ml-6">{plan.description}</span>
                  )}
                </div>
          ...
          <div className="bg-white/5 rounded-lg p-4 border border-white/10">
            ...
            <div className="flex flex-col gap-3">
              <Button 
                ...
                className={`w-full ${selectedPlan ? "bg-green-600 hover:bg-green-700" : "bg-green-600/70 hover:bg-green-700/70"} text-white gap-2`}
              ...
              >
                ...
                {selectedPlan ? "WhatsApp" : "Выберите тариф"}
              </Button>
